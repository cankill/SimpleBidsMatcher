...
Необходимо указать следующие параметры для запуска приложения:

```
  -c, --clients <file>  clients is an required input file with clients accounts
  -r, --result <file>   result is an optional output file for storing results accounts state of orders matching (default: result.txt)
  -o, --orders <file>   orders is a required input file with orders
```

Для standalon запуска необходимo в classpath добавить библиотеку "com.github.scopt" %% "scopt" % "3.6.0"
